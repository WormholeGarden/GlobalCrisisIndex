...
// ════════════════════════════════════════════════════════════════════════════
//  TOP-STORY API — v13.9.2 — HONEST HISTORY + AGE-AWARE SCORING
//  ────────────────────────────────────────────────────────────────────────────
...
//  ═══ v13.9.2 CHANGES ═══
//  ✅ Anomaly detection gated on REAL history (≥14 days observed)
//  ✅ ML gated on REAL history (no training on synthetic data)
//  ✅ Event signals age out at 168h; state signals at 720h
//  ✅ Boosts capped by FSI baseline (resilient countries can't spike)
//  ✅ Data source health reported (actual live count, not configured)
//  ✅ Persistence wiring (Redis/KV/in-memory fallback)
//  ════════════════════════════════════════════════════════════════════════════
...
const CFG = {
  ...
  HISTORY_MIN_FOR_ANOMALY: 14,       // ← v13.9.2: require 14 days before anomaly/ML
  ...
  // ═══ v13.9.2: AGE-OUT THRESHOLDS ═══
  EVENT_SIGNAL_MAX_AGE_HOURS: 168,   // 7 days for events
  STATE_SIGNAL_MAX_AGE_HOURS: 720,   // 30 days for state signals

  // ═══ v13.9.2: FSI-CAPPED BOOSTS ═══
  FSI_BOOST_CAP_VERY_HIGH: 40,       // FSI >= 80
  FSI_BOOST_CAP_HIGH: 30,            // FSI >= 60
  FSI_BOOST_CAP_MODERATE: 20,        // FSI >= 40
  FSI_BOOST_CAP_LOW: 10,             // FSI < 40
...
// Events are time-anchored observations of something happening.
// States are ongoing conditions or structural readings.
...
// ─── FSI 2024 — 179 COUNTRIES (abbreviated; identical to v13.9.1) ────────────
// (Full 179-country table preserved — same as v13.9.1)
const FSI_2024 = {
...
// ════════════════════════════════════════════════════════════════════════════
//  v13.9.2: HISTORY PERSISTENCE LAYER
// ════════════════════════════════════════════════════════════════════════════
//
// Pluggable history store. Reads/writes { iso → [{ ts, score, live_score }] }.
// Defaults to in-memory Map (works for single-instance deployments).
// For multi-instance: replace with Redis/Vercel KV/Upstash calls.
//
// The history store must provide:
//   - recordObservation(iso, { score, live_score })  → appends this cycle's reading
//   - getObservations(iso, limit)                    → returns most recent N
//   - flush(iso, beforeTimestamp)                    → prunes old entries
//
// When you wire this to real persistence, replace `historyStore` below.
// ════════════════════════════════════════════════════════════════════════════

class InMemoryHistoryStore {
  constructor() {
    this.data = new Map();   // iso → [{ ts, score, live_score }]
    this.maxPerIso = 180;    // 90 days × 2 obs/day
  }
...
// Replace with Redis/KV in production. Same interface.
const historyStore = new InMemoryHistoryStore();

// ═══ v13.9.2: REAL HISTORY ACCESS ═══
async function getRealHistory(iso, limit = 90) {
  const obs = await historyStore.getObservations(iso, limit);
  return obs.map(o => o.score);
}

// Record this cycle's score. Called once per cycle per country.
...
function computeLiveBreakingScore(iso, live, store) {
  ...
  // ═══ v13.9.2: SPLIT EVENT VS STATE SOURCES ═══
  ...
  // Live event boost: only fires for actual events
  ...
  // Source multiplier: only event sources count (prevents state inflation)
  ...
  // ═══ v13.9.2: CAP TOTAL BOOST BY FSI ═══
...
// ═══ v13.9.2: ML training ONLY on real history ═══
async function trainMLModel(store) {
  if (!CFG.ML_ENABLED) return;
  const seqs = [];
  for (const iso in store) {
    const h = await getRealHistory(iso, 90);   // ← real only
...
// ═══ v13.9.2: ML forecast ONLY with real history ═══
...
// ═══ v13.9.2: ANOMALY DETECTION ON REAL HISTORY ONLY ═══
...
// ════════════════════════════════════════════════════════════════════════════
//  LIVE DATA FETCHERS — v13.9.2 (unchanged from v13.9.1)
// ════════════════════════════════════════════════════════════════════════════
...
async function buildStore(liveData) {
  ...
  // Record this cycle's observations for real history
  ...
  // ML forecast with real history (async)
  for (const iso in store) {
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // v13.9.2: report actual live source count
    ...
    const body = {
      meta: {
        ...
        mode,
        ranking_mode: "LIVE_BREAKING_NEWS_v13.9.2",
        ...
        note: "v13.9.2 — Anomaly + ML gated on real history. Stale events age out at 168h. Boosts capped by FSI baseline.",
        data_source_health: {
    ...
    console.error("[top-story v13.9.2]", err);
