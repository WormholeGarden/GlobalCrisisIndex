...
// ════════════════════════════════════════════════════════════════════════════
//  TOP-STORY API — v13.9.5 — BEST SCORED VERSION
//  ────────────────────────────────────────────────────────────────────────────
...
//  ═══ All v13.9.1 features preserved ═══
//  ✅ Event deduplication
//  ✅ Real history persistence
//  ✅ Anomaly gating (INSUFFICIENT_HISTORY)
//  ✅ ML gating (no synthetic training)
...
// ═══ RANKING — effective_score primary, has_fresh tiebreaker, live_score, freshness ═══
...
async function buildStore(liveData) {
  ...
  // Compute effective score: max(structural, live)
  for (const iso in store) {
    ...
    if (CFG.SCORE_FIELD_IS_LIVE) store[iso].score = store[iso].__live_breaking?.live_score || store[iso].score;
  }
  ...
  try {
    ...
    const body = {
      meta: {
        ...
        note: "v13.9.5 — Best scored version. Ranking: effective_score → has_fresh → live_score → freshness. Event dedup + real history preserved.",
        live_news_stats: {
