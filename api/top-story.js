...
// ════════════════════════════════════════════════════════════════════════════
//  TOP-STORY API — v13.9.3 — SINGLE-COUNTRY CONSISTENCY FIX
//  ────────────────────────────────────────────────────────────────────────────
...
//  ═══ v13.9.3 CHANGES ═══
//  ✅ Single-country path uses store's __live_breaking (no re-computation)
//  ✅ Palestine name-matching expanded (Gaza, West Bank, OPT, cities)
//  ✅ Anomaly gate enforces methods_fired >= 2 AND history >= 14
//  ✅ Score field reports live_score even when 0 (no structural fallback)
//  ✅ Cross-endpoint consistency check in meta
//  ═══ v13.9.2 CHANGES (preserved) ═══
...
const REGION_KEYWORDS = {
  ...
  SYR: ['syria','damascus','aleppo','idlib','homs','latakia','raqqa','deir ez-zor'],
  UKR: ['ukraine','kyiv','kiev','kharkiv','odesa','donetsk','luhansk','crimea','zaporizhzhia','mariupol'],
  AFG: ['afghanistan','kabul','kandahar','herat','mazar-i-sharif','kunduz'],
  COD: ['congo','kinshasa','kivu','katanga','ituri','goma','bukavu'],
  SOM: ['somalia','mogadishu','kismayo','baidoa','hargeisa'],
  MLI: ['mali','bamako','timbuktu','gao','kidal'],
  NGA: ['nigeria','lagos','abuja','borno','yobe','adamawa'],
  ETH: ['ethiopia','addis ababa','tigray','amhara','oromia','somali region'],
  MMR: ['myanmar','burma','yangon','mandalay','rakhine','shan','kachin'],
  HTI: ['haiti','port-au-prince','port au prince','cap-haitien'],
  VEN: ['venezuela','caracas','maracaibo','zulia'],
};
...
function matchesCountryPlace(iso, place) {
  ...
  if (p.includes(countryName)) return true;
...
function runAnomalyDetection(realHistory) {
  ...
  return {
    detected,
    severity: !detected ? "NONE" : (f.length >= 4 ? "EXTREME" : f.length >= 3 ? "CRITICAL" : "HIGH"),
    reason: null,
...
function extractSignals(iso, live) {
  ...
  const ifrcEvents = (live.ifrc.data || []).filter(ev => {
    ...
    return iso3 === iso || matchesCountryPlace(iso, ev.name || "");
  });
  ...
  const ifrcAppeals = (live.ifrcAppeals?.data || []).filter(ap => {
    ...
    return iso3 === iso || matchesCountryPlace(iso, ap.name || "");
  });
  ...
  if (CFG.WHO_DON_ENABLED && live.whoDon.data?.length) {
    const matched = live.whoDon.data.filter(d => {
      ...
      return text.includes(name) || matchesCountryPlace(iso, text);
    });
  ...
  const diseaseRow = (live.disease.data || []).find(d => {
    ...
    return cN.toLowerCase() === name || matchesCountryPlace(iso, cN);
  });
